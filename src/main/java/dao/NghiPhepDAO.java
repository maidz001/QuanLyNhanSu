...
public class NghiPhepDAO {
    ...
    public List<NghiPhep> layTatCa() {
        ...
        String sql = "SELECT np.*, nv.ho_ten, nv.ma_nhan_vien, nd.ho_ten as ten_nguoi_duyet FROM nghi_phep np LEFT JOIN nhan_vien nv ON np.nhan_vien_id=nv.nhan_vien_id LEFT JOIN nhan_vien nd ON np.nguoi_duyet=nd.nhan_vien_id ORDER BY np.nghi_phep_id DESC";
    ...
    public List<NghiPhep> layTheoNhanVien(int nhanVienId) {
        List<NghiPhep> ds = new ArrayList<>();
        String sql = "SELECT np.*, nv.ho_ten, nv.ma_nhan_vien, nd.ho_ten as ten_nguoi_duyet FROM nghi_phep np LEFT JOIN nhan_vien nv ON np.nhan_vien_id=nv.nhan_vien_id LEFT JOIN nhan_vien nd ON np.nguoi_duyet=nd.nhan_vien_id WHERE np.nhan_vien_id=? ORDER BY np.nghi_phep_id DESC";
