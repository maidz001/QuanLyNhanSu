...
public class TaiKhoanDAO {
    ...
    public TaiKhoan dangNhap(String tenDangNhap, String matKhau) {
        String sql = "SELECT tk.*, nv.ho_ten FROM tai_khoan tk LEFT JOIN nhan_vien nv ON tk.nhan_vien_id=nv.nhan_vien_id WHERE tk.ten_dang_nhap=? AND tk.mat_khau=? AND tk.trang_thai=1";
    ...
    public List<TaiKhoan> layTatCa() {
        ...
        String sql = "SELECT tk.*, nv.ho_ten FROM tai_khoan tk LEFT JOIN nhan_vien nv ON tk.nhan_vien_id=nv.nhan_vien_id";
    ...
    public TaiKhoan layTheoId(int id) {
        String sql = "SELECT tk.*, nv.ho_ten FROM tai_khoan tk LEFT JOIN nhan_vien nv ON tk.nhan_vien_id=nv.nhan_vien_id WHERE tk.tai_khoan_id=?";
        try (Connection conn = DBConnection.layKetNoi(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return mapRow(rs);
    ...
    private TaiKhoan mapRow(ResultSet rs) throws SQLException {
        ...
        try { tk.setHoTen(rs.getString("ho_ten")); } catch (SQLException ignored) {}
        return tk;
    }
}