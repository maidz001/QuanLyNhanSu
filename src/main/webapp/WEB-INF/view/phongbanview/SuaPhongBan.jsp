...
<div class="card">
    ...
    <div class="card-body">
        <form action="${pageContext.request.contextPath}/phongban" method="post">
            ...
            <input type="hidden" name="id" value="${phongban.phongBanId}"/>

            <div class="form-grid">

                <div class="form-group">
                    <label>Mã phòng ban</label>
                    <input type="text" value="${phongban.maPhongBan}" readonly/>
                </div>

                <div class="form-group">
                    <label>Tên phòng ban <span style="color:var(--danger)">*</span></label>
                    <input type="text" name="tenPhongBan" value="${phongban.tenPhongBan}" required/>
                </div>

                <div class="form-group">
                    <label>Trưởng phòng</label>
                    <select name="truongPhongId">
                        <option value="">${truongPhongHienTai}</option>
                        <c:forEach var="nv" items="${listNhanVien}">
                            <option value="${nv.nhanVienId}"
                                ${nv.nhanVienId == phongban.truongPhongId ? "selected" : ""}>
                                ${nv.hoTen}
                            </option>
                        </c:forEach>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="trangThai" required>
                        <option value="1" ${phongban.trangThai == 1 ? "selected" : ""}>Hoạt động</option>
                        <option value="0" ${phongban.trangThai == 0 ? "selected" : ""}>Ngưng hoạt động</option>
                    </select>
                </div>

                <div class="form-group full">
                    <label>Mô tả</label>
                    <textarea name="moTa">${phongban.moTa}</textarea>
                </div>

            </div>

            <div class="form-footer">
                <a href="${pageContext.request.contextPath}/phongban" class="btn btn-outline">
